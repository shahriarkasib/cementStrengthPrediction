...
class Model_Finder:
    ...
    def get_best_params_for_linearReg(self,train_x,train_y):
        ...
        try:
            # initializing with different combination of parameters
            data_db = {'objective': 'linearregbestparams', 'status': 'ok', 'error': '',
            ...
            data_db = {'objective': 'XGBoostbestparams', 'status': 'ok', 'error': '',
                       'message': 'Finished Linear regression training',
                       'time': dt.now().strftime("%d/%m/%Y %H:%M:%S")}
            self.db_obj.insert_data(data_db)
            return self.linReg
        except Exception as e:
            data_db = {'objective': 'linearregbestparams', 'status': 'error', 'error': 'ExceptionError',
                       'message': str(e),
                       'time': dt.now().strftime("%d/%m/%Y %H:%M:%S")}
            self.db_obj.insert_data(data_db)
            raise Exception()
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            data_db = {'objective': 'linearregerror', 'status': 'ok', 'error': '',
                       'message': self.LinearReg_error,
                       'time': dt.now().strftime("%d/%m/%Y %H:%M:%S")}
            self.db_obj.insert_data(data_db)
         ...
         # create best model for XGBoost
            ...
            data_db = {'objective': 'randomforesterror', 'status': 'ok', 'error': '',
                       'message': self.prediction_randomForestReg_error,
                       'time': dt.now().strftime("%d/%m/%Y %H:%M:%S")}
            self.db_obj.insert_data(data_db)
